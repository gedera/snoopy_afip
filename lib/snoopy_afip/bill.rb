...
module Snoopy
  class Bill
    attr_accessor :total_net, :document_num, :document_type, :concept, :currency, :result, :issuer_iva_cond,
                  :due_date, :aliciva_id, :cbte_asoc_num, :cae, :service_date_to,
    ...
    ATTRIBUTES_PRECENSE = [:total_net, :concept, :receiver_iva_cond, :alicivas, :document_type, :document_num, :service_date_from, :service_date_to, :cuit, :sale_point, :issuer_iva_cond]
    ...
    #   raise Snoopy::AfipWsfeTimeout.new
    # rescue => e
    #   raise Snoopy::Exception::AfipWsfeTimeout.new
    # end
    ...
    def validate_attributes_presence
      ...
      if _attrs.present?
        exception = Snoopy::Exception::Bill::MissingAttributes.new(_attrs.join(', '))
        @errors << exception
        raise exception
      end
    end
    ...
    def validate_standar_values
      unless Snoopy::CURRENCY.keys.include?(@currency.to_sym)
        exception = Snoopy::Exception::Bill::InvalidValueAttribute.new(":currency. Possible values #{Snoopy::CURRENCY.keys}")
        @errors << exception
        raise exception
      end

      unless Snoopy::IVA_COND.include?(@issuer_iva_cond.to_sym)
        exception = Snoopy::Exception::Bill::InvalidValueAttribute.new(":issuer_iva_cond. Possible values #{Snoopy::IVA_COND}")
        @errors << exception
        raise exception
      end

      unless Snoopy::BILL_TYPE.keys.include?(@receiver_iva_cond.to_sym)
        exception = Snoopy::Exception::Bill::InvalidValueAttribute.new(":receiver_iva_cond. Possible values #{Snoopy::BILL_TYPE.keys}")
        @errors << exception
        raise exception
      end

      unless Snoopy::DOCUMENTS.keys.include?(@document_type)
        exception = Snoopy::Exception::Bill::InvalidValueAttribute.new(":document_type. Possible values #{Snoopy::DOCUMENTS.keys}")
        @errors << exception
        raise exception
      end

      unless Snoopy::CONCEPTS.keys.include?(@concept)
        exception = Snoopy::Exception::Bill::InvalidValueAttribute.new(":concept. Possible values #{Snoopy::CONCEPTS.keys}")
        @errors << exception
        raise exception
      end
    end
